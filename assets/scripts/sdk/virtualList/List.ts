...
import { Node, Component, Enum, tween,Event as CCEvent, EventTouch, _decorator, EventHandler, Tween, ScrollView, Prefab, Layout, Vec2, Size, NodePool, isValid, instantiate, Vec3, Widget, UITransform, CCFloat, CCBoolean, CCInteger } from 'cc';
...
export default class List extends Component {
    ...
    _onTouchStart(ev: EventTouch, captureListeners) {
        ...
        //注意这里要修改为CCEvent.AT_TARGET
        let isMe = ev.eventPhase === CCEvent.AT_TARGET && ev.target === this.node;
        if (!isMe) {
    ...
    _onTouchCancelled(ev: EventTouch, captureListeners) {
        let t = this;
